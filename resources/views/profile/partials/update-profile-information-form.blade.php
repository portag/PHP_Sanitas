<section>
    <header>
        ...
        <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">
            {{ __("Actualiza tu información básica aquí") }}
        </p>
    </header>
    ...
    <form method="post" action="{{ route('profile.update') }}" class="mt-6 space-y-6">
        ...
        @if (Auth::user()->rol == 'medico' || Auth::user()->rol == 'admin')
            <div>
                <x-input-label for="especialidad" :value="__('Especialidad')" />
                <select name="especialidad" id="especialidad" class="mt-1 block w-full">
            ...
            @else
            <div>
                <input type="hidden" name="especialidad" value="{{Auth::user()->especialidad}}">
            </div>
        @endif
